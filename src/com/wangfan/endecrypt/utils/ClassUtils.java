...
import com.wangfan.endecrypt.exception.InstantiationException;
import com.wangfan.endecrypt.exception.UnknownClassException;
...
public class ClassUtils {
    ...
    public static InputStream getResourceAsStream(String name) {
        ...
        if (is == null) {
            System.out.println("Resource [" + name + "] was not found via the thread context ClassLoader.  Trying the " + "current ClassLoader...");
            is = CLASS_CL_ACCESSOR.getResourceStream(name);
        }

        if (is == null) {
        	System.out.println("Resource [" + name + "] was not found via the current class loader.  Trying the " +
                        "system/application ClassLoader...");
            is = SYSTEM_CL_ACCESSOR.getResourceStream(name);
        }

        if (is == null) {
        	System.out.println("Resource [" + name + "] was not found via the thread context, current, or " +
    ...
    public static Class forName(String fqcn) throws UnknownClassException {
        ...
        if (clazz == null) {
        	System.out.println("Unable to load class named [" + fqcn +
                        "] from the thread context ClassLoader.  Trying the current ClassLoader...");
            clazz = CLASS_CL_ACCESSOR.loadClass(fqcn);
        }

        if (clazz == null) {
        	System.out.println("Unable to load class named [" + fqcn + "] from the current ClassLoader.  " +
    ...
    private static abstract class ExceptionIgnoringAccessor implements ClassLoaderAccessor {
        ...
        public Class loadClass(String fqcn) {
            ...
            if (cl != null) {
                try {
                	...
                	System.out.println("Unable to load clazz named [" + fqcn + "] from class loader [" + cl + "]");
                }
            }
            return clazz;
        }
        ...
        protected final ClassLoader getClassLoader() {
            try {
            	...
            	System.out.println("Unable to acquire ClassLoader.");
            	t.printStackTrace();
            }
            return null;
        }
